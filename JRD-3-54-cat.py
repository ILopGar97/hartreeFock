...
if __name__ == "__main__":
    ...
    if Q == 0:
        ...
        ext = "ani"
    alpha_values = [0.5, 0.75, 0.9, 1, 2] # <---------- Aquí incluimos el/los valor/es de alpha/q que queremos estudiar para JRDq (si vas a incluir valores enteros como el 1, pon el punto i.e 1.0). También puedes definirlo con [num/den for num in range(ini, final-1)]
    if Q == 0: 
        rg = range(3, 55)
    else:
        rg = "all"
    ...
    for a in alpha_values:
        for atom in atoms_selected:
            tasks += [ #<----- Aqui estamos haciendo una lista con las tareas que vamos a realizar para poder paralelizarlo, en este caso el separamos el calculo de JRD en espacios y en valores de a y atomos
                (f"JRD posicion - alpha={a:.3f} -"+ext, JenRenyiDivergence, {"atoms": atom, "alpha": a, "space": "position"}), #<---- saldrá un warning indicando que por defecto se ha cogido el espacio indicado (ignorar)
                (f"JRD momento - alpha={a:.3f} -"+ext, JenRenyiDivergence, {"atoms": atom, "alpha": a, "space": "momentum"})
            ]
    ...
    with open("resultsJRD-"+ext+".json", "w") as json_file: #<------------- guardamos el json
        json.dump(resultsJRD, json_file, indent=4)

    json_to_multiple_txt("resultsJRD-"+ext+".json")  #<--------- lo ponemos bonito y ordenados en tablas (se genera una carpeta nueva, pueden salir desordenadas, se arreglará pronto... Mientras tengamos el json todo ok)
