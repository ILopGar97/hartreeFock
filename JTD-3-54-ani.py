...
if __name__ == "__main__":
    ...
    atoms_selected = atoms_selected[0:4]
    ...
    for a in alpha_values:
        tasks += [
            (f"JTD - posicion alpha={a:.3f}-ani", JenTsallisDivergence, {"atoms": atoms_selected, "alpha": a, "space": "position"}),
            (f"JTD - momento alpha={a:.3f}-ani", JenTsallisDivergence, {"atoms": atoms_selected, "alpha": a, "space": "momentum"})
        ]
    ...
    with open("resultsJTD-ani.json", "w") as json_file:
        json.dump(resultsJTD, json_file, indent=4)

    json_to_multiple_txt("resultsJTD-ani.json")
