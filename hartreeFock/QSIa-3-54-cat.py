...
if __name__ == "__main__":
    ...
    Q = 1 #<---- aqui elige que quieres estudiar neutros = 0, aniones = -1, cationes = 1. No te salgas de esos valores, si no dará error o calculará isoelectronicas. 
    if Q == 0:
        ...
        ext = "ani"
    alpha_values = [num / 4 for num in range(2, 13)] # <---------- Aquí incluimos el/los valor/es de alpha/q que queremos estudiar para JRDq (si vas a incluir valores enteros como el 1, pon el punto i.e 1.0). También puedes definirlo con [num/den for num in range(ini, final-1)]
    if Q == 0: 
        rg = range(3, 55)
    else:
        rg = "all"
    ...
    if Q != 0:
        atoms_selected = atoms_selected[3:] #<---- para los aniones descartamos el hidrogeno y el helio
    ...
    for a in alpha_values:
        tasks += [ #<----- Aqui estamos haciendo una lista con las tareas que vamos a realizar para poder paralelizarlo, en este caso el separamos el calculo de JRD en espacios y en valores de a y atomos
            (f"QSIa posicion - alpha={a:.3f} -"+ext, QSI_alpha, {"atoms": atoms_selected, "alpha": a, "space": "position"}), #<---- saldrá un warning indicando que por defecto se ha cogido el espacio indicado (ignorar)
            (f"QSIa momento - alpha={a:.3f} -"+ext, QSI_alpha, {"atoms": atoms_selected, "alpha": a, "space": "momentum"})
        ]
    ...
    with open("resultsQSIa-"+ext+".json", "w") as json_file: #<------------- guardamos el json
        json.dump(resultsJRD, json_file, indent=4)

    json_to_multiple_txt("resultsQSIa-"+ext+".json")  #<--------- lo ponemos bonito y ordenados en tablas (se genera una carpeta nueva, pueden salir desordenadas, se arreglará pronto... Mientras tengamos el json todo ok)
