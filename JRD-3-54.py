...
if __name__ == "__main__":
    ...
    if Q == 0:
        ...
        ext = "ani"
    alpha_values = [0.5] # <---------- Aquí incluimos el/los valor/es de alpha/q que queremos estudiar para JRDq (si vas a incluir valores enteros como el 1, pon el punto i.e 1.0). También puedes definirlo con [num/den for num in range(ini, final-1)]
    if Q == 0: 
        rg = range(3, 55)
    else:
        rg = "all"
    atoms_list = load_system(0, BASE_PATH) #<-------- Aquí se puede cambiar los sitemas que queremos estudiar si queremos estudiar atomos neutros 0, -1 aniones, +1 cationes (con eso hacemos referencia a la carga). Si introducimos un número entre 2 y 9 iremos a las series isoelectronicas (estamos indicando el número de electrones).
